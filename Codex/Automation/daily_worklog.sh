...
# 由 daily_run.sh 编排调用（也可独立跑）。设计：确定性抽数 + 无头 codex 只格式化补写。
#   周一–周五：抽当天对话摘要 + 读当天文件变更记录行 → 无头 codex 写 简洁+详细 两档 → 只补缺失行
#   周六       ：不跑（周末并入周日）
#   周日       ：兜底补齐本周缺失的工作日行 + 写"周末"行（周六+周日合并）
# 铁律 write-if-missing：只补该文件本周"缺失"的天行，绝不覆盖已存在行（护人工精修/手写历史）。
# 分工：extract_day.py 抽对话摘要（确定性）；无头 codex 只格式化 + 用 Write 补写。
#
# 用法：
...
if wait_proxy; then
  log "代理就绪（或无需代理），调用无头 codex 写工作总结…"
  CL_OUT=$(call_codex "$PROMPT"); CL_RC=$?
  log "codex 结果 rc=$CL_RC 输出=${CL_OUT}"
else
  log "代理离线，跳过写工作总结（缺失天由次日回看补齐）"
fi

# 幂等兜底提交（无头 codex 的 Write 通常已由 git hook 自动 commit；这里再兜一次，无改动则跳过）
