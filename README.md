...
### 5.5. 信任 Codex hooks（Codex 用户必做）

Codex 有 hook trust 安全机制，hooks 配置后默认不执行。必须手动信任：

1. 启动 Codex 后打开设置（Settings）
2. 找到 hooks 列表，对工作区的 hooks 选择“信份”
3. 重启 Codex 后 hooks 生效

> 不信任的后果：S 级读隔离、自动提交、审计日志全部失效。
