...
8. <span style="color:#228B22">🟢 每次工作**先读本侧 `待同步.md`**（变更请求收件箱：使用本模板的下游实际工作区会话把该改的事交办到这里）。挑"待处理"条目落地后：实际改 → 写 `../Releases.md` / `../变更详情.md` → 把该条状态改"已落地（vX.X.X）"并移入其存档表 → commit / push。</span>
